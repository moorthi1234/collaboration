function test(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("example"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "test"));
