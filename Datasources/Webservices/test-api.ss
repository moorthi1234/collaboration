function api(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("test"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "api"));
