function rest2(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("sample"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "rest2"));
