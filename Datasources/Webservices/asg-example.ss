function example(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("asg"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "example"));
